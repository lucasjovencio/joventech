    ...
    public function getSubCategoriasDataTable(Object $request,$id)
    {
        ...
        return DataTables::of($subcategorias)
                ...
                ->addColumn('action', function($row){
                    $btn = '<a href="#" data-nome="'.($row->nome ?? '').'" data-toggle="modal" id="" data-slug="'.($row->slug ?? '').'" data-target="#update" data-src="'.route('sub-categoria.update',['sub_categorium'=>$row->id]).'" onClick="update(this)" class="edit btn btn-primary btn-sm">Editar</a>';
